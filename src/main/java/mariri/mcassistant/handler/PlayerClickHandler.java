...
public class PlayerClickHandler {
	...
	public void onPlayerRightClick(PlayerInteractEvent.RightClickBlock e){
		if(!isProcessing.contains(e.getEntityPlayer()) && !e.getEntityPlayer().world.isRemote && e.getEntityPlayer().isSneaking() == SNEAK_INVERT){
...
//	        	ChunkCoordinates respawn = new ChunkCoordinates(e.x, e.y, e.z);
	            if (	world.provider.canRespawnHere() &&
	                ...
	                e.getEntityPlayer().sendStatusMessage(new TextComponentString(BEDASSIST_SET_RESPAWN_MESSAGE), true);
	                e.setCanceled(e.getEntityPlayer().trySleep(e.getPos()) != EntityPlayer.SleepResult.OK);
	            }
			}
			// 寝るの禁止
			if(BEDASSIST_NO_SLEEP){
                e.getEntityPlayer().sendStatusMessage(new TextComponentString(BEDASSIST_NO_SLEEP_MESSAGE), true);
                e.setCanceled(true);
			}
		}
		// 農業補助機能
			...
			if(CROPASSIST_AREA_ENABLE && Lib.compareCurrentToolLevel(e.getEntityPlayer(), CROPASSIST_AREA_REQUIRE_TOOL_LEVEL)){
				...
				for(int xi = -1 * area; xi <= area; xi++){
					for(int zi = -1 * area; zi <= area; zi++){
						...
						Block b = s.getBlock();
						if(block == b){
							b.onBlockActivated(world, newpos, s, e.getEntityPlayer(), hand, e.getFace(), 0, 0, 0);
							count++;
						}
					}
				}
		...
		// 耕地化補助機能
			...
			for(int xi = -1 * area; xi <= area; xi++){
				for(int zi = -1 * area; zi <= area; zi++){
					...
					if(current.getItem().onItemUse(e.getEntityPlayer(), world, new BlockPos(e.getPos().getX() + xi, e.getPos().getY(), e.getPos().getZ() + zi), hand, e.getFace(), 0, 0, 0) == EnumActionResult.SUCCESS){
						count++;
					}
				}
			}
		...
		// トーチ補助機能
			...
			if(index >= 0){
				...
				if(		!world.getBlockState(e.getPos()).getBlock().onBlockActivated(world, e.getPos(), world.getBlockState(e.getPos()), e.getEntityPlayer(), EnumHand.MAIN_HAND, e.getFace(), 0, 0, 0) &&
						current.getItem().onItemUse(e.getEntityPlayer(), world, e.getPos(), EnumHand.MAIN_HAND, e.getFace(), 0, 0, 0) == EnumActionResult.PASS &&
						torch.getItem().onItemUse(e.getEntityPlayer(), world, e.getPos(), EnumHand.MAIN_HAND, e.getFace(), 0, 0, 0) == EnumActionResult.SUCCESS){
					e.getEntityPlayer().setHeldItem(EnumHand.MAIN_HAND, heldItem);
